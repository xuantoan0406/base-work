<template>
  <div class="lineWorkplace">
    <div
      v-for="workplace in user.workplaceByDate"
      :key="workplace.id"
      class="line"
      :style="{backgroundColor:workplace.place.color,
               width:calWidth(workplace.created_at, workplace.time_finish)+'%',
               left:calLeft(workplace.created_at)+'%'}"
    >
      <div class="place" :style="{backgroundColor: workplace.place.color}">
        <div class="content" :style="{backgroundColor: workplace.place.color}">
          <div class="circle" />
...
export default {
  ...
  created() {
    if (this.user.workplaceByDate[Object.keys(this.user.workplaceByDate)[Object.keys(this.user.workplaceByDate).length - 1]]) {
      this.user.workplaceByDate[Object.keys(this.user.workplaceByDate)[Object.keys(this.user.workplaceByDate).length - 1]].time_finish = moment().format('YYYY-MM-DD HH:mm:ss');
    }
  },
  mounted() {
    // this.getworkplaceByDate();
  ...
  methods: {
    ...
    //     this.$http.post('/api/getworkplaceByDate', {date: this.date, userID: this.user.id })
    //         .then( response => {
    //             this.condition = true;
    //             this.lineworkplaceDays = response.data.data;
    //             console.log(this.lineworkplaceDays);
    //         })
    //         .catch(error => {
    //            error.response.data;
    //         });
    ...
    CalPercentHours() {
      //   console.log('CalPercentHours');
      // const lineworkplaceRow = document.getElementsByClassName('lineworkplaceRow');
      // const cell = lineworkplaceRow[0].getBoundingClientRect();
      // const lineworkplaceRowWidth = cell.width;
      // console.log(lineworkplaceRowWidth);
      //
      // const cellPixel = lineworkplaceRowWidth / 13;
      // console.log(cellPixel);
      // this.PercentHour = parseFloat(cellPixel) / lineworkplaceRowWidth * 100;
